...
class WideResNet(nn.Module):
    def __init__(self, n=3, num_classes=1, widen_factor=1, drop_rate=0.0):
    ...
    def forward(self, x):
        out = self.conv1(x)  # 32, 16, 128, 128
        out = self.block1(out)  # 32, 64, 128, 128
        out = self.block2(out)  # 32, 128, 64, 64
        out = self.block3(out)  # 32, 256, 32, 32
        out = self.relu(self.bn1(out))  # 32, 256, 32, 32
        out = F.avg_pool2d(out, 8)  # 64, 256, 4, 4
        print(out.shape)
        out = out.view(-1, 256 * 4 * 4)  # 1024, 256 -> 64, (256 * 4 * 4)
        print(out.shape)
        return self.fc(out)  # 512, 2 -> 32, 2
